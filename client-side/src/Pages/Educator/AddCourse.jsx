...
const AddCourse = () => {
  ...
  const handleChapter = (action, chapterId) => {
    if (action === "add") {
      ...
      if (title) {
        const newChapter = {
          chapterId: Date.now() + Math.floor(Math.random() * 1000),
      ...
      chapters.map((chapter) => {
        if (chapter.chapterId === currentChapterId) {
          const newLecture = {
            ...
            lectureId: Date.now() + Math.floor(Math.random() * 1000)
          ...
          {showpops && (
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <button onClick={addLecture} className="w-full bg-blue-400 text-white px-4 py-2 rounded" type="button">
                  Add
                </button>

                <img
                  onClick={() => setShowPops(false)}
                  src={assets.cross_icon}
                  alt="close"
